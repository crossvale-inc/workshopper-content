...
button, then click the *"CI\CD"* link. Find the `jenkins-ephemeral` template, and click on it:
...
You can ignore any warning about granting permissions to your project.
...
If this fails, saying the rolebinding "jenkins_edit" already exists, you can ignore that error.
...
Use the following values in their respective fields (be particularly careful to change the Database Service Name and MongoDB Database Name since they default to "mongodb" and "sampledb" respectively):
...
prevent two deployments, we will need to disable automatic deployments.

Edit the deployment config for the nationalparks application, to uncheck the "Automatically start a new deployment when the image changes" option at:

[source,role=copypaste]
----
https://{{CONSOLE_ADDRESS}}/console/project/{{EXPLORE_PROJECT_NAME}}{{USER_SUFFIX}}/edit/dc/nationalparks
----

image::disable-automatic-deployment.png[Disable Automatic Deployment]
...
In order to create the pipeline, click on the *Add to project* button, then click the *Browse Catalog*, and search to find the `dev-live-pipeline` template, and click on it. Specify the "* Project name" (i.e. the name of the project where your
`nationalparks` *Dev* and *Live* containers are deployed, in the format `{{EXPLORE_PROJECT_NAME}}{{USER_SUFFIX}}`)
and click on *Create*.
