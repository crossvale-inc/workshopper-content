...
is a compressed archive.  When you uncompress the download, it will create a folder starting with "openshift-origin-client-tools-v3.7.1-ab0f056". I would suggest moving the contents of that folder (including the oc executable) to the relevant directory below:
...
At this point, oc version may return an ip error because you have not logged in yet. Later, when you oc login and do the version command again it will not give this error.
...
WARNING: Earlier versions of the Openshift Client will not work with this lab.
