...
again click the *"Add to Project"* button, then *"Browse Catalog"*. Click the *Databases* link, or search for `mongodb` in the search bar.

image::mongodb-datastores.png[Data Stores]

Scroll down to find the *MongoDB (Ephemeral)* template, and click it.  You will notice that there are several
...
now, be sure to use the following values in their respective fields (be particularly careful to change the Database Name since it defaults to "sampledb"):
